    ...
    <div className="hidden min-h-screen lg:flex lg:w-auto lg:flex-col">
      ...
      <div className="flex min-h-0 flex-1 flex-col bg-gray-800">
        <div className="flex h-16 flex-shrink-0 items-center bg-gray-900 px-4">
          <Link href={'/'} className="flex items-center inline-block text-white font-bold text-lg space-x-2 scale-[0.85]">
            <Image
              ...
              return item.children && item.children.length > 0 ?
                ...
                <div key={item.name} className="w-full">
                  <button
                    key={item.name}
                    onClick={() => {
                      if(activeDropDown === item.name) setActiveDropDown(undefined)
                       else {
...

                    {
                      (
                        <motion.div
                          initial={{ height: 0 }}
                          animate={{
                            height: activeDropDown === item.name ? item.children.length * 38 : 0,
                            transition: {
                            duration: 0.06,
                            ease: 'linear'
                          }}}
